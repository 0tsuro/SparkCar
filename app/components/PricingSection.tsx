...
const VEHICLES = [
  { id: "citadine", label: "Citadine", emoji: "🚙", desc: "Clio, 208, Polo..." },
...
const FORMULA_META: Record<string, { label: string; highlight: boolean; features: string[] }[]> = {
  exterieur: [
    {
      label: "Classique",
      highlight: true,
...
const CAL_CONFIGS: Record<string, { namespace: string; calLink: string }> = {
  "exterieur-express":             { namespace: "express-exterieur",    calLink: "https://cal.com/sparkcar/exterieur-classique" },
  "exterieur-approfondi":          { namespace: "approfondi-exterieur", calLink: "https://cal.com/sparkcar/exterieur-premium" },
  "exterieur-premium":             { namespace: "premium-exterieur",    calLink: "https://cal.com/sparkcar/exterieur-prestige" },
  "interieur-express":             { namespace: "express-interieur",    calLink: "https://cal.com/sparkcar/express-interieur" },
  "interieur-approfondi":          { namespace: "approfondi-interieur", calLink: "https://cal.com/sparkcar/approfondi-interieur" },
  "interieur-premium":             { namespace: "premium-interieur",    calLink: "https://cal.com/sparkcar/premium-interieur" },
  "exterieur-interieur-express":   { namespace: "express-complet",      calLink: "https://cal.com/sparkcar/pack-classique" },
  "exterieur-interieur-approfondi":{ namespace: "approfondi-complet",   calLink: "https://cal.com/sparkcar/pack-premium" },
  "exterieur-interieur-premium":   { namespace: "premium-complet",      calLink: "https://cal.com/sparkcar/pack-prestige" },
};

const WASH_TYPES = [
  { id: "exterieur",          label: "Extérieur",           emoji: "🚿", desc: "Lavage de la carrosserie" },
  { id: "interieur",          label: "Intérieur",           emoji: "🧹", desc: "Nettoyage de l'habitacle" },
  { id: "exterieur-interieur",label: "Extérieur + Intérieur", emoji: "✨", desc: "Lavage complet" },
];
...
const STEP_LABELS = ["Votre véhicule", "Type de lavage", "Votre formule", "Votre créneau"];

type CalConfig = { namespace: string; calLink: string };

function CalEmbed({ config, cacheKey }: { config: CalConfig; cacheKey: string }) {
  useEffect(() => {
    (async () => {
...
export default function PricingSection() {
  ...
  const selectedFormulaIndex = selectedFormula ? FORMULA_IDS.indexOf(selectedFormula as typeof FORMULA_IDS[number]) : -1;
  const selectedFormulaMeta = formulaMetas && selectedFormulaIndex >= 0 ? formulaMetas[selectedFormulaIndex] : null;
  const selectedPrice = getPrice(selectedVehicle, selectedWashType, selectedFormulaIndex);

  const calConfigKey = selectedWashType && selectedFormula ? `${selectedWashType}-${selectedFormula}` : null;
  const calConfig = calConfigKey ? CAL_CONFIGS[calConfigKey] : null;

  const canAdvance =
    step === 0 ? !!selectedVehicle :
    step === 1 ? !!selectedWashType :
    !!selectedFormula;
    ...
    <section className="w-full bg-gradient-to-b from-gray-50 to-white py-8 md:py-24 px-6 md:px-8 flex flex-col items-center">
      ...
      >
        ...
        <p className="text-gray-600 text-sm md:text-lg max-w-2xl mx-auto leading-relaxed">
          Sélectionnez votre véhicule et votre formule, puis choisissez un créneau disponible.
        </p>
      </motion.div>
      ...
      <div className="w-full max-w-5xl">
        <AnimatePresence mode="wait">
          ...
          {/* Step 1 – Vehicle */}
          {step === 0 && (
            <motion.div key="vehicle" variants={stepVariants} initial="initial" animate="animate" exit="exit">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-4 mb-4 md:mb-10">
                {VEHICLES.map((v) => (
                  ...
                  >
                    ...
                    <span className="font-bold text-gray-900 text-sm md:text-base text-center leading-tight">{v.label}</span>
                    <span className="hidden md:block text-xs text-gray-500 text-center leading-snug">{v.desc}</span>
                  </motion.button>
                ))}
              </div>
            </motion.div>
          ...
          {/* Step 2 – Wash type */}
          {step === 1 && (
            <motion.div key="washtype" variants={stepVariants} initial="initial" animate="animate" exit="exit">
              <h3 className="text-lg md:text-2xl font-bold text-gray-800 mb-4 md:mb-8 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4 mb-4 md:mb-10">
                {WASH_TYPES.map((w) => (
                  ...
                  >
                    ...
                    <span className="text-3xl md:text-4xl select-none">{w.emoji}</span>
                    <span className="font-bold text-gray-900 text-sm md:text-lg">{w.label}</span>
                    <span className="hidden md:block text-xs text-gray-500 text-center leading-snug">{w.desc}</span>
                  </motion.button>
                ))}
              </div>
            </motion.div>
          ...
          {/* Step 3 – Formula */}
          {step === 2 && formulaMetas && (
            <motion.div key="formula" variants={stepVariants} initial="initial" animate="animate" exit="exit">
              <h3 className="text-lg md:text-2xl font-bold text-gray-800 mb-4 md:mb-8 text-center">
          ...
          {step === 3 && selectedFormulaMeta && calConfig && (
            <motion.div key="calendar" variants={stepVariants} initial="initial" animate="animate" exit="exit">
              ...
              <div className="flex items-center justify-center gap-2 md:gap-3 mb-4 md:mb-8 flex-wrap">
                <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-1.5 md:gap-2">
                  ...
                  <span className="font-semibold text-blue-800 text-xs md:text-sm">{vehicle?.label}</span>
                </div>
                <ChevronRight className="w-3 h-3 md:w-4 md:h-4 text-gray-400" />
                <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-1.5 md:gap-2">
                  <span className="text-base md:text-xl">{washType?.emoji}</span>
                  <span className="font-semibold text-blue-800 text-xs md:text-sm">{washType?.label}</span>
                </div>
                <ChevronRight className="w-3 h-3 md:w-4 md:h-4 text-gray-400" />
                <div className="bg-blue-50 border border-blue-200 rounded-xl px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-1.5 md:gap-2">
                  <span className="font-semibold text-blue-800 text-xs md:text-sm">{selectedFormulaMeta.label}</span>
